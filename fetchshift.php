...
$sql ="SELECT s.id, s.user_id, s.start, s.finish, s.shift_break, u.id, u.organisation_id, u.name, 
...
//Search
$sql = "SELECT s.id, s.user_id, s.start, s.finish, s.shift_break, u.id, u.organisation_id, u.name, 
            o.id, o.oname,o.hourly_rate from users as u join shifts as s ON u.id = s.user_id left join organisations as o on u.organisation_id = o.id where u.organisation_id = '$jid' and 1=1";
...
while($row=mysqli_fetch_array($query)){
    ...
    $subdata[]='<button type="button" id="getEdit" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#myModal" data-id="'.$row[0].'"><i class="fa fa-edit">&nbsp;</i>Edit</button>
                <button type="button" id="getDelete" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#myModal" data-id="'.$row[0].'><i class="fa fa-trash">&nbsp;</i>Delete</button>';
    $data[]=$subdata;
}
